  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-2">
        {orderIndex !== null && (
          <span className="shrink-0 w-4 h-4 rounded-full bg-white/15 text-[9px] font-bold flex items-center justify-center text-white/80 mt-0.5">
            {orderIndex + 1}
          </span>
        )}
      ...
      {/* Description */}
      <p className="text-[9px] text-muted-foreground/50 leading-relaxed line-clamp-2">
        {node.description}
      </p>
      {/* Bottom: impact + icons */}
...
function ApplyPlanPanel({ plan, graph }: { plan: ApplyPlan; graph: OptimizationGraph }) {
  ...
  return (
    <div className="bg-[#05080c] border border-white/[0.08] rounded-xl p-4 space-y-3">
      <div className="flex items-center gap-2">
        <Zap size={13} className="text-cyan-400" />
        <span className="text-xs font-semibold">GameMode 適用プラン</span>
        <span className="text-[10px] text-muted-foreground/40 ml-auto">KILL + POWER + WINDOWS + NETWORK</span>
      </div>
        ...
        {plan.order.map((id, i) => {
          ...
          return (
            <div key={id} className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border ${sty.border} ${sty.bg}`}>
              <span className="text-[9px] text-muted-foreground/50 tabular-nums font-mono">{i + 1}.</span>
  ...
  return (
    <div className={`bg-[#05080c] border ${sty.border} rounded-xl p-4 space-y-3`}>
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] text-muted-foreground/40 font-mono ml-auto">{node.id}</span>
      </div>
      <p className="text-xs text-muted-foreground/60">{node.description}</p>
      <div className="grid grid-cols-3 gap-3 text-[10px]">
        <div className="space-y-0.5">
          <p className="text-muted-foreground/40">カテゴリ</p>
          <p className={sty.color}>{sty.label}</p>
        </div>
        <div className="space-y-0.5">
          <p className="text-muted-foreground/40">推定効果</p>
          <p className="text-foreground font-semibold">{node.estimated_impact} pt</p>
        </div>
        <div className="space-y-0.5">
          <p className="text-muted-foreground/40">権限</p>
      ...
      {(outEdges.length > 0 || inEdges.length > 0) && (
        <div className="space-y-1.5">
          <p className="text-[10px] text-muted-foreground/40 uppercase tracking-wider">エッジ</p>
          {inEdges.map((e, i) => (
  ...
  return (
    <div className="p-5 flex flex-col gap-4 h-full overflow-y-auto">
      ...
      <div className="flex items-center gap-5 px-1">
        <span className="text-[10px] text-muted-foreground/40 uppercase tracking-wider">凡例</span>
        ...
        <div className="flex items-center gap-1.5 ml-4">
          <span className="w-4 h-4 rounded-full bg-white/15 text-[9px] font-bold flex items-center justify-center text-white/80">1</span>
