/**
 * OptimizationGraphView  ESprint 5 / S5-02
 *
 * Optimization Graph DAG の可視化ペ�Eジ、E
 * - ノ�EドをカチE��リ別カラムに配置�E�Eosition: absolute + SVG エチE��オーバ�Eレイ�E�E
 * - REQUIRES / CONFLICTS / SUGGESTS エチE��を色刁E��して描画
 * - ApplyPlan パネルで適用頁E��確誁E
 * - ノ�EドクリチE��で詳細パネルを表示
 */
...
> = {
  ...
  power:    { label: "電溁E,           color: "text-amber-400",  border: "border-amber-500/35",  bg: "bg-amber-500/8",  dot: "#fbbf24", headerBg: "bg-amber-500/8"  },
...
const EDGE_CONFIG: Record<EdgeType, { stroke: string; dash: string; label: string; markerColor: string }> = {
  requires:  { stroke: "#60a5fa", dash: "",      label: "忁E��依孁E, markerColor: "#60a5fa" },
  conflicts: { stroke: "#f87171", dash: "5,3",   label: "競吁E,     markerColor: "#f87171" },
  suggests:  { stroke: "#34d399", dash: "4,4",   label: "提桁E,     markerColor: "#34d399" },
};
    ...
    >
      ...
      <div className="flex items-center justify-between gap-1 mt-0.5">
        ...
        <div className="flex items-center gap-1 text-muted-foreground/55">
          {node.requires_admin && <ShieldAlert size={9} aria-label="要管琁E��E />}
    ...
    <div className="bg-[#05080c] border border-white/[0.12] rounded-xl p-4 space-y-3">
      ...
      {plan.suggestions.length > 0 && (
        <p className="text-[10px] text-emerald-400/60">
          提案ノーチE {plan.suggestions.join(", ")}
        </p>
      )}
      {plan.conflicts.length > 0 && (
        <p className="text-[10px] text-red-400/60">
          競合スキチE�E: {plan.conflicts.map(c => `${c.node_a} ↁE${c.node_b}`).join(", ")}
        </p>
      )}
    </div>
  );
    ...
    <div className={`bg-[#05080c] border ${sty.border} rounded-xl p-4 space-y-3`}>
      ...
      <div className="grid grid-cols-3 gap-3 text-[10px]">
        <div className="space-y-0.5">
          <p className="text-muted-foreground/55">カチE��リ</p>
          <p className={sty.color}>{sty.label}</p>
        </div>
        <div className="space-y-0.5">
          <p className="text-muted-foreground/55">推定効极E/p>
          <p className="text-foreground font-semibold">{node.estimated_impact} pt</p>
        </div>
        <div className="space-y-0.5">
          <p className="text-muted-foreground/55">権陁E/p>
          <p className={node.requires_admin ? "text-amber-400" : "text-emerald-400"}>
            {node.requires_admin ? "要管琁E��E : "不要E}
          </p>
        </div>
      </div>
      {(outEdges.length > 0 || inEdges.length > 0) && (
        <div className="space-y-1.5">
          <p className="text-[10px] text-muted-foreground/55 uppercase tracking-wider">エチE��</p>
          {inEdges.map((e, i) => (
            <div key={`in-${i}`} className="flex items-center gap-2 text-[11px]">
              <span className="text-muted-foreground/55">ↁE/span>
  ...
  return (
    <div className="p-5 flex flex-col gap-4 h-full overflow-y-auto">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-bold tracking-tight">最適化グラチE/h1>
          <p className="text-[11px] text-muted-foreground/50 mt-0.5">
            {graph.nodes.length} ノ�EチE· {graph.edges.length} エチE�� · DAG 依存解決
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-5 px-1">
        <span className="text-[10px] text-muted-foreground/55 uppercase tracking-wider">凡侁E/span>
        ...
        <div className="flex items-center gap-1.5 ml-4">
          ...
          <span className="text-[10px] text-muted-foreground/60">適用頁E/span>
        </div>
      </div>
      ...
      {selectedId && (
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[11px] text-muted-foreground/50">ノ�Eド詳細</span>
          </div>
