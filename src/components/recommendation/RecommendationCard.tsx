/**
 * RecommendationCard  E推奨エンジン V2 ウィジェチE��
 *
 * ENABLE_RECOMMENDATION_V2 ぁEtrue のとき�Eみ HomeHub から呼び出される、E
 *
 * 表示フロー:
 *   idle ↁEintent 選抁E+ 「推奨を取得」�Eタン
 *   loading ↁEスピナー
 *   result ↁERecommendationItem カード一覧 + summary
 *   error ↁEエラーメチE��ージ�E�ENABLE_RECOMMENDATION_V2 = false 時�Eガイドを含む�E�E
 */
...
const INTENTS: { value: RecommendationIntent; label: string; desc: string }[] = [
  { value: "fps",       label: "FPS最大匁E,   desc: "最高フレームレート優允E },
  { value: "stability", label: "安定性優允E,   desc: "クラチE��ュ・フリーズを防ぁE },
  { value: "silence",   label: "静音・省E��劁E, desc: "ファンノイズと消費電力を抑えめE },
...
function ImpactPill({ item }: { item: RecommendationItem }) {
  ...
  return <span className="flex items-center gap-0.5 text-[10px] text-muted-foreground/55"><Minus size={9} /> E/span>;
}
      ...
      {!loading && error && (
        <div className="flex items-start gap-2 p-3 bg-red-500/8 border border-red-500/20 rounded-xl">
          ...
          <div className="flex-1 min-w-0">
            {isFlagDisabled ? (
              <>
                <p className="text-xs font-medium text-red-300">推奨エンジン V2 は無効でぁE/p>
                <p className="text-[10px] text-muted-foreground/50 mt-0.5">
                  recommendation.rs の ENABLE_RECOMMENDATION_V2 めEtrue に設定してください、E
                </p>
              </>
            ) : (
              <>
