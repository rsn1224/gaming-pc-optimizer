    ...
    <div className="flex flex-col items-center text-center gap-5">
      ...
      <div className="flex flex-wrap justify-center gap-3 text-xs text-muted-foreground/60">
        {[
          ...
          <span key={f} className="flex items-center gap-1.5 px-3 py-1 bg-white/[0.04] border border-white/[0.08] rounded-full">
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl p-4 space-y-3">
        <label className="text-xs font-medium text-muted-foreground/70">
          OpenAI API キー <span className="text-muted-foreground/40">（任意）</span>
        </label>
        <div className="flex gap-2">
          <div className="relative flex-1">
            <input
              ...
              placeholder="sk-..."
              className="w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2 text-sm font-mono pr-9 outline-none focus:border-cyan-500/40 transition-colors"
            />
            <button
              type="button"
              onClick={() => setShow((v) => !v)}
              className="absolute right-2.5 top-1/2 -translate-y-1/2 text-muted-foreground/40 hover:text-white transition-colors"
            >
      ...
      <div className="w-full bg-white/[0.02] border border-white/[0.06] rounded-xl p-4 text-left space-y-1.5">
        ...
        {[
          ...
          <div key={i} className="flex items-center gap-2 text-xs text-muted-foreground/70">
            <span className="w-4 h-4 flex items-center justify-center bg-cyan-500/10 border border-cyan-500/20 rounded-full text-[9px] text-cyan-400 font-bold shrink-0">
              {i + 1}
            </span>
            {s}
          </div>
...
export function FirstRunWizard() {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="relative w-full max-w-md mx-4 bg-[#05080c] border border-white/[0.08] rounded-2xl overflow-hidden shadow-[0_24px_64px_rgba(0,0,0,0.8)]">
        {/* Top accent line */}
