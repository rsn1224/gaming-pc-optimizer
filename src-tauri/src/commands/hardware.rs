...
pub struct GpuStatus {
    pub name: String,
    /// GPU ベンダー: "nvidia" | "amd" | "intel" | "unknown"
    pub vendor: String,
...
/// nvidia-smi → WMI フォールバックの順で GPU 情報を取得する。
pub(crate) fn fetch_gpu_status_sync() -> Result<Vec<GpuStatus>, String> {
    // 1st: nvidia-smi（NVIDIA GPU）
    if let Ok(gpus) = fetch_nvidia_gpu() {
        return Ok(gpus);
    }
    // 2nd: WMI フォールバック（AMD / Intel / その他）
    fetch_generic_gpu()
}
...
fn fetch_nvidia_gpu() -> Result<Vec<GpuStatus>, String> {
...
/// 温度・電力・ファン速度は取得不可のため 0 を返す。
fn fetch_generic_gpu() -> Result<Vec<GpuStatus>, String> {
    let script = r#"
Get-WmiObject Win32_VideoController |
  Where-Object { $_.Name -notmatch 'Microsoft|Remote|Virtual|Basic' } |
  ForEach-Object {
    $vram = if ($_.AdapterRAM -gt 0) { [math]::Round($_.AdapterRAM / 1MB) } else { 0 }
    "$($_.Name)|$vram|$($_.DriverVersion)"
  }
"#;
    ...
    if !output.status.success() {
        return Err("WMI GPU query failed".to_string());
    }
    ...
    if gpus.is_empty() {
        Err("GPU情報を取得できませんでした".to_string())
    } else {
        Ok(gpus)
    }
}
